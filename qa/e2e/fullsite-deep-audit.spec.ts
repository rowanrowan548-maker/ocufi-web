...
test.describe('T-QA-FULLSITE-DEEP-AUDIT · 阶段 2 · 14 页渲染 + 关键交互', () => {
  ...
  test.describe('/ landing', () => {
    ...
    test('Hero 搜索框 / · 输 BONK · modal 出 ≥ 1 结果(R3 修后 search 已接通 birdeye)', async ({ page, request }) => {
      // 先 sanity check 后端真有数据(避免前端 hook 实现 bug 误判)
      ...
      // 前端 modal 真出结果(R3-FE ship `ecf54c9` 后走后端 endpoint)
      await gotoAndSettle(page, withPreview('/'));
      // "/" 触发全局搜索 modal(SPEC §B.1 + user-reported-bugs case 6 同 pattern)
      await page.evaluate(() => {
      ...
      await page.waitForFunction(
        () => {
          ...
          return buttons.some((b) => b.offsetParent !== null && (b.textContent ?? '').toLowerCase().includes('bonk') && !!b.querySelector('[class*="tabular"]'));
        },
        undefined,
        { timeout: 8_000 },
      );
