...
export function ComingSoonScreen() {
  const t = useTranslations('launch.comingSoon');
  ...
  return (
    <main className="flex flex-1 flex-col items-center justify-center px-4 py-12 sm:py-16 relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl flex flex-col items-center text-center gap-6">
        ...
        {/* 主标语 */}
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight font-heading leading-[1.05] whitespace-pre-line">
          {t('title')}
        </h1>

        <p className="text-base sm:text-lg text-muted-foreground max-w-xl leading-relaxed">
          {t.rich('subtitle', {
            b: (chunks) => <strong className="text-foreground/90 font-semibold">{chunks}</strong>,
          })}
        </p>

        {/* 4 个差异化点 */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 w-full pt-4">
          <FeatureChip label={t('chips.fee')} />
          <FeatureChip label={t('chips.nonCustodial')} />
          <FeatureChip label={t('chips.openSource')} />
          <FeatureChip label={t('chips.onChain')} />
        </div>
        ...
        <div className="flex gap-3 text-[10px] text-muted-foreground/50 mt-4 flex-wrap justify-center">
          <Link href="/legal/privacy" className="hover:text-muted-foreground">{t('footer.privacy')}</Link>
          <span>·</span>
          <Link href="/legal/terms" className="hover:text-muted-foreground">{t('footer.terms')}</Link>
          <span>·</span>
          <Link href="/legal/disclaimer" className="hover:text-muted-foreground">{t('footer.disclaimer')}</Link>
        </div>

        <div className="text-[10px] font-mono text-muted-foreground/40 mt-2 inline-flex items-center gap-1">
          <Lock className="h-2.5 w-2.5" />
          {t.rich('previewHint', {
            key: (chunks) => <span className="text-foreground/70">{chunks}</span>,
          })}
        </div>
      </div>
    </main>
